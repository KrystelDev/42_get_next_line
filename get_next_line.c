...
/*   Updated: 2023/10/01 17:51:14 by kryrodri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 1- Hay algo pendiente a devolver ? >> while hasta que sea Sí o Nada que leer
    ...
    {
        sub_string = ft_substr(reading, 0, ft_look_for_line(reading)+1);
        if (!sub_string)
        {
            ft_clean(&result);
        }
        ...
        {
            result = ft_strjoin(result, sub_string);
            ft_clean(&sub_string);

        }
        if (!result)
        {
            // ft_clean(&reading); //no soluciona nada
            return (NULL);
        }
